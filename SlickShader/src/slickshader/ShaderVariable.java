...
/**
 * Class used to keep track of variables associated with this shader.</br>
 * 
...
class ShaderVariable {
	public enum Qualifier {
		...
		private static final Map<String, ShaderVariable.Qualifier> stringToEnum = new HashMap<String, ShaderVariable.Qualifier>();
	...
	public enum Type {
		BOOLEAN("boolean"), DOUBLE("double"), FLOAT("float"), INTEGER("integer");
	...
	ShaderVariable(int programID, String name, ShaderVariable.Qualifier qual, ShaderVariable.Type type, int vecSize,
			int size) {
	...
	void setUniformValue(boolean[] vals) {
		...
		switch (vecSize) {
		case 1:
			GL20.glUniform1(location, fb);
			break;
		case 2:
			GL20.glUniform2(location, fb);
			break;
		case 3:
			GL20.glUniform3(location, fb);
			break;
		case 4:
			GL20.glUniform4(location, fb);
			break;
		}
	}
	...
	private void locationCheck() {
		if (location == -1 && !isCulled) {
			System.err.println("Location for variable " + name + "could not be found.\nGLSL may remove "
					+ "any vairable that does not contribute " + "to an output. Check and ensure " + "that " + name
					+ "is an active variable.\n");
			isCulled = true;
		}
	}

	public float getUniformFloatValue() {
		if (this.type != Type.FLOAT) {
			System.err.printf(TYPE_WARN, this.name, this.type, Type.FLOAT);
		}
		if (this.qualifier != Qualifier.UNIFORM) {
