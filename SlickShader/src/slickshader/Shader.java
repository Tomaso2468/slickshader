...
/**
 * Class used to use and access shaders without having to deal with all of the
 * fidly openGL bits.
 * 
...
public class Shader {
	...
	/**
	 * ID of the <tt>Shader</tt>. A Shader may have programID of -1 only before
	 * construction is completed, or after the <tt>Shader</tt> is deleted
	 */
	private int programID = NOT_LOADED;
	private Map<String, ShaderVariable> vars = new HashMap<String, ShaderVariable>();

	private Shader(ShaderResourceManager srm, Collection<URL> vertex, Collection<URL> fragment) throws SlickException {
		this.srm = srm;
	 ...
	 */
	public static Shader makeShader(URL vertexFileName, URL fragmentFileName) throws SlickException {
		ArrayList<URL> l1 = new ArrayList<URL>();
		l1.add(vertexFileName);
		ArrayList<URL> l2 = new ArrayList<URL>();
		l2.add(fragmentFileName);

		return new Shader(ShaderResourceManagerImpl.getSRM(), l1, l2);
	}
	...
	/**
	 * Sets the number of characters to be returned when printing errors.</br>
	 * Suggested values are the constants <tt>BRIEF</tt>, <tt>MODERATE</tt>, and
	 * <tt>VERBOSE</tt>.</br>
	 * 
	 * @param detailLevel number of characters to display for error messages.
	 */
	 ...
	 * TODO should this be called from <tt>finalise()</tt>, or is that just asking
	 * for trouble?
	 */
	...
	public void startShader() {
		if (programID == NOT_LOADED) {
			throw new IllegalStateException("Cannot start shader; this" + " Shader has been deleted");
		}
...
//UNIFORM SETTERS  
	...
	public Shader setUniformIntVariable(String name, int v0, int v1, int v2) {
		return setUniformIntVariable(name, new int[] { v0, v1, v2 });
	}

	public Shader setUniformIntVariable(String name, int v0, int v1, int v2, int v3) {
	...
	public Shader setUniformFloatVariable(String name, float v0, float v1) {
		return setUniformFloatVariable(name, new float[] { v0, v1 });
	}

	public Shader setUniformFloatVariable(String name, float v0, float v1, float v2) {
		return setUniformFloatVariable(name, new float[] { v0, v1, v2 });
	}

	public Shader setUniformFloatVariable(String name, float v0, float v1, float v2, float v3) {
	...
	public Shader setUniformMatrix(String name, boolean transpose, float[][] matrix) {
		...
		case 2:
			GL20.glUniformMatrix2(location, transpose, matBuffer);
			break;
		case 3:
			GL20.glUniformMatrix3(location, transpose, matBuffer);
			break;
		case 4:
			GL20.glUniformMatrix4(location, transpose, matBuffer);
			break;
		}
...
//    return GL20.glGetShader(programID, GL20.GL_LINK_STATUS)==GL11.GL_TRUE;
	...
	private void scrapeVariables(String varLine) {
		...
		if (str.equals("float")) {
			...
			case 'b':
				...
				break;
			case 'd':
				type = ShaderVariable.Type.DOUBLE;
				break;
			case 'i':
			case 'u':
				type = ShaderVariable.Type.INTEGER;
		...
		while (scanner.hasNext("[\\w_]+[\\w\\d_]*(\\[\\d+\\])?")) {
			...
			ShaderVariable var = new ShaderVariable(programID, name, qualifier, type, vecSize, size);
	...
	private void scanSource(URL filename) throws SlickException {
		Scanner scanner;
		try {
	...
	public static Shader makeShader(String string, String string2) throws SlickException {
		return makeShader(Shader.class.getResource(string), Shader.class.getResource(string2));
	}
	...
	public float getUniformFloatVariable(String name) {
		ShaderVariable var = vars.get(name);
		return var.getUniformFloatValue();
	}
}
